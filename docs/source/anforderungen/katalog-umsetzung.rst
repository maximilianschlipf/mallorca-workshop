...
Sechs der Tests bestehen bereits im Backend, in
``SchulungsFilterApiTest``. Sie sind hier nicht neu erfunden, sondern
nachträglich an die Anforderungen angebunden -- die Suchfunktion war
umgesetzt und getestet, bevor sie beschrieben war.
...
.. test:: Ohne Angaben umfasst das Ergebnis den ganzen Katalog
   ...
   Besteht als ``shouldReturnAllSchulungenWithoutParams``.
...
.. test:: Suche ignoriert Schreibweise und Leerraum
   ...
   Besteht als ``shouldSearchCaseInsensitiveAndTrimmed``.
...
.. test:: Filter trifft die Kategorie genau
   ...
   Besteht als ``shouldFilterByKategorie``.
...
.. test:: Suche und Filter greifen gemeinsam
   ...
   Besteht als ``shouldCombineSucheAndKategorie``.
...
.. test:: Kein Treffer liefert eine leere Liste
   ...
   Besteht als ``shouldReturnEmptyArrayWhenNoMatch``.
...
.. test:: Kategorienliste ist doppelfrei und sortiert
   ...
   Besteht als ``shouldReturnDistinctSortedKategorien``.
